...
import { PostInterface, SinglePostResponse } from "../types";
          ...
          <div className="w-full flex justify-between content-center">
            ...
            <div>
              <button
                ...
                className="mr-4 focus:shadow-outline-teal inline rounded-lg border border-transparent bg-teal-600 px-4 py-2 text-sm font-medium leading-5 text-white shadow transition-colors duration-150 hover:bg-teal-700 focus:outline-none dark:hover:bg-teal-500"
              >
                Update
              </button>
              <button
          ...
          <header>
            <div className="space-y-1 border-b pt-6 md:pt-8 border-gray-200 pb-10 text-center dark:border-gray-700">
              <div className="text-base font-medium leading-6 text-gray-500 dark:text-gray-400">
                {postData.updated_at
                  ? `Updated on ${getReadableData(postData.created_at)}`
                  : `Published on ${getReadableData(postData.created_at)}`}
              </div>
              <div>
              ...
              <div className="text-left">
                <Link to={`/posts/${responseData?.previous_post.slug}`}>
                  <span className="text-sm md:text-base font-normal no-underline hover:underline">
                    &larr; {truncateText(responseData.previous_post.title, 20)}
                  </span>
                  <br />
                </Link>
              </div>
